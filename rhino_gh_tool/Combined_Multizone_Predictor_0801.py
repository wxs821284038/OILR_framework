...
def predict_one_case(root_path, GCN_model, Outdoor_model, In_mag_model, In_hres_model, Regularizer_model, ext_nxy, ext_nz, nxy, nz, delta):
    SYM_NORM = False
    start = time.time()

    # get room dimensions and opening dimensions from the csv files, NOTE: opening csv does not contain flow direction info
    room_param = get_room_parameters(root_path)
    ...
    GCN_start = time.time()
    pred = GCN_model.predict([room_matrix, graph_conv_filters])[0]
    ...
    ext_start = time.time()
    ext_pred_mx = Outdoor_model.predict(ext_bnd_mx)
    ...
    for rm_ind in reordered_rm_inds:
        ...
        mag_start = time.time()
        rm_mag_mx = In_mag_model.predict([rm_bnd_mx, xyz_deltas])
        print("Room #" + str(rm_ind) + " Magnitude model finished prediction in " + str(int(time.time()-mag_start)) +" seconds.")

        # use the boundary matrix, xyz deltas and magnitude field to predict the u v w p 4 channel flow field
        uvwp_start = time.time()
        rm_pred_mx = In_hres_model.predict([rm_bnd_mx, rm_mag_mx, xyz_deltas])
    ...
    print("Reg bnd mx finished in " + str(int(time.time()-reg_bnd)) +" seconds.")
    
    reg_start = time.time()
    regularizer_output = Regularizer_model.predict(regularizer_input)[0]
...
if __name__ == '__main__':
    ...
    GCN_load_model = load_model(GCN_filename, custom_objects={'MultiGraphCNN': MultiGraphCNN}, compile=False)

    # load Outdoor model
    Outdoor_filename = root_path + "models/Outdoor_model_0801_000085.h5"
    Outdoor_load_model = load_model(Outdoor_filename, compile=False)

    # load Indoor model
    In_mag_filename = root_path + "models/Indoor_model_mag_0801_000280.h5"
    In_mag_load_model = load_model(In_mag_filename, compile=False)

    In_hres_filename = root_path + "models/Indoor_model_uvwp_0801_000225.h5"
    In_hres_load_model = load_model(In_hres_filename, compile=False)

    # load Regularize model
    Regularizer_filename = root_path + "models/regularizer_gen_model_0801_000058.h5"
    Regularizer_load_model = load_model(Regularizer_filename, compile=False)

    predicted_flow_pattern = predict_one_case(root_path, GCN_load_model, Outdoor_load_model, In_mag_load_model, In_hres_load_model, Regularizer_load_model, ext_nxy, ext_nz, nxy, nz, delta)

    for i in range(ext_nz):
        u_2d = predicted_flow_pattern[:,:,i,0]
        v_2d = predicted_flow_pattern[:,:,i,1]
        w_2d = predicted_flow_pattern[:,:,i,2]
        p_2d = predicted_flow_pattern[:,:,i,3]
        np.savetxt(root_path + "Result_uvwp/result_u_" + str(i) + ".csv", u_2d, delimiter=",", fmt='%10.4f')
        np.savetxt(root_path + "Result_uvwp/result_v_" + str(i) + ".csv", v_2d, delimiter=",", fmt='%10.4f')
        np.savetxt(root_path + "Result_uvwp/result_w_" + str(i) + ".csv", w_2d, delimiter=",", fmt='%10.4f')
        np.savetxt(root_path + "Result_uvwp/result_p_" + str(i) + ".csv", p_2d, delimiter=",", fmt='%10.4f')
